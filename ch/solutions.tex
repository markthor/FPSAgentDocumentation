...
\begin{figure}[H]
    \centering
    	\begin{scriptsize}
		\sffamily
    	\includesvg[svgpath = img/, width = 1\textwidth]{verticalangle}
    	\end{scriptsize}
        \vspace{-10mm}
...
There are multiple approaches to overcome these imbalances.
One method is to delete data from overrepresented classes until every class has an equal amount of samples. This can severely affect the amount of training data as data is deleted until every class has samples equal to the lowest denominator. A second method is proposed in~\cite{balanced-classes}, showing promising results. The idea is to randomly duplicate samples from lesser represented classes, until classes are more evenly distributed. The most extreme form of this method duplicates samples until an even distribution is achieved.




%Seeing these results it was clear that class imbalances was a thing to consider.
